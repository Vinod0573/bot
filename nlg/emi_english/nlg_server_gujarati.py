...
async def generate_response(nlg_call):
    ...
    import json
    utterance = 'Gujarati response'
    with open("Sheet1.json", "r+", encoding='utf-8') as f:
        data = json.load(f)
    # with open("commercial_vehicle_responses.json", "r+", encoding='utf-8') as f:
    #     data1 = json.load(f)
    # data+=data1
    for item in data:
        if item and "Response_ID" in item and item["Response_ID"] == template and \
            ...
            if "buttons" in item and item["buttons"] and item["buttons"] == "":
                ...
                x = ast.literal_eval(item["buttons"]) if "buttons" in item and item["buttons"] else []
...
def run_server(port, workers):
    ...
    async def nlg(request):
        ...
        print("NLG Response gujarati", nlg_call)
