...
async def generate_response(nlg_call):
    ...
    for item in data:
        ...
        if item and "Response_ID" in item and item["Response_ID"] == template and (utterance in item or utterance1 in item):
            # print("Utterance Template:",item['Utterance Template'])
            if "buttons" in item and item["buttons"] and item["buttons"] == "":
                for x in item:
                    if x[utterance]!="":
                        text="".join(x[utterance],'')
                    else:
                        text="".join(x[utterance1],'')
        ...
        # if item and "Utterance Template" in item and item["Utterance Template"] == template and (utterance in item):
        #     if "buttons" in item and item["buttons"] and item["buttons"] == "":
        #         for x in item:
        #             if x[utterance]!="":
        #                 text="".join(x[utterance],'')
