...
class AvailFinancePostEMI(FormAction):
    def name(self):  # type: () -> Text
        return "post_emi_7_updated_form"
    ...
    def slot_mappings(self):  # type: () -> Dict[Text: Union[Dict, List[Dict]]]
        return {
            ...
            "ask_delay_reason": [
                ...
                self.from_intent(intent ="inform",value = "+1"),
            ...
            if self._should_request_slot(tracker, slot):
                if slot == "availability_status":
                    ...
                    if trail_count is None:
                        ...
                        if total_loans>1:
                            dispatcher.utter_template("utter_greet_ml_b_post_due",tracker,total_amount=str(total_emi_amount),no_of_loans=str(total_loans))
                            ...
                            send_and_store_disposition_details(tracker=tracker,dispatcher=dispatcher,disposition_id="pic",sheet_name=sheet_name,emi_flow=emi_flow)
                        return [FollowupAction("action_listen"), SlotSet(REQUESTED_SLOT, slot),SlotSet("timestamp", time.time()),SlotSet("trail_count",trail_count+1)]
                    else:
                ...
                if slot=="payment_confirmation":
                    if trail_count is None:
                        ...
                        return [FollowupAction("action_listen"), SlotSet(REQUESTED_SLOT, slot),SlotSet("timestamp", time.time()),
    ...
    def validate_availability_status(
        ...
        if value=="FALSE":
            ...
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,
                                               disposition_id="rtp", flag=DEFAULT_FLAG,emi_flow=emi_flow,sheet_name=sheet_name)
                ...
                if intent in date_check_intents:
                    ...
                    for entity in entities:
                        if entity.get("entity", None) == "date":
                            ...
                            if given_date:
                                ...
                                if no_of_days.days==0:
                                    ...
                                    send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher,
                                                                    ...
                                                                    disposition_id="PTP",
                                                                    flag=TIMEOUT_FLAG, ptp_date=given_date,emi_flow=emi_flow,sheet_name=sheet_name)
                                    # send_message(tracker)
                                    return {"availability_status": value, "stop_conversation": "TRUE", "trail_count": None}
                                    ...
                                    send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher,
                                                                    user_message=user_message,
                                                                    disposition_id="PTP",
                                                                    flag=DEFAULT_FLAG, ptp_date=given_date,emi_flow=emi_flow,sheet_name=sheet_name)
                                    # send_message(tracker)
                            ...
                            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,
                                                disposition_id="agree to pay", flag=DEFAULT_FLAG,emi_flow=emi_flow,sheet_name=sheet_name)
                            return {"availability_status": "confirm", "trail_count": None}
            else:
                print("here ******************")
            ...
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,disposition_id="delay reason",
                                               ...
                                               flag=DEFAULT_FLAG,emi_flow=emi_flow,sheet_name=sheet_name)
            return {"availability_status": "confirm", "trail_count": None}
        ...
        if value=="FALSE":
            ...
            if entities:
                ...
                send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher,
                                                user_message=user_message,
                                                disposition_id="PTP",
                                                flag=TIMEOUT_FLAG, emi_flow=emi_flow,sheet_name=sheet_name,ptp_date=give_date)
                # send_message(tracker)
                return {"payment_confirmation": value, "stop_conversation": "TRUE", "trail_count": None}
        elif value == "decline_reason":
            ...
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,disposition_id="delay reason",
                                               delay_reason=decline_reason_disposition_id.get(intent),
                                               flag=TIMEOUT_FLAG,emi_flow=emi_flow,sheet_name=sheet_name)
    ...
    def validate_ask_delay_reason(
        ...
        if value == "FALSE":
            ...
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,disposition_id="RTP",
                                               flag=TIMEOUT_FLAG, delay_reason=decline_reason_disposition_id.get(intent),emi_flow=emi_flow,sheet_name=sheet_name)
