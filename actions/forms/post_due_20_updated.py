...
class AvailFinancePostEMI(FormAction):
    def name(self):  # type: () -> Text
        return "post_emi_20_updated_form"
    ...
    def required_slots(tracker: Tracker) -> List[Text]:
        ...
        if stop_conversation == "TRUE":
            return []
        if payment_confirmation == "decline":
            return ["ask_delay_reason"]
        if availability_status=="confirm":
        ...
        for slot in self.required_slots(tracker):
            ...
            if self._should_request_slot(tracker, slot):
                ...
                if slot=="payment_confirmation":
                    if trail_count is None:
                        trail_count=0
                        dispatcher.utter_template("utter_persue_to_pay_post_due8_15",tracker)
                        return [FollowupAction("action_listen"), SlotSet(REQUESTED_SLOT, slot),SlotSet("timestamp", time.time()),SlotSet("sheet_name",sheet_name),SlotSet("emi_flow",emi_flow),
                                SlotSet("trail_count",trail_count+1)]
                    else:
                        dispatcher.utter_template("utter_repeat_ask_payment_1_common",tracker)
    ...
    def validate_availability_status(
        ...
        if value=="FALSE":
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,
                                               disposition_id="rtp", flag=DEFAULT_FLAG,emi_flow="post_emi_7_15",sheet_name=sheet_name)
            return {"availability_status": "confirm"}
        elif value=="TRUE":
            ...
            if entities:
                if intent in date_check_intents:
                    ...
                    for entity in entities:
                            ...
                            if given_date:
                                ...
                                if no_of_days.days==0:
                                    ...
                                    send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher,
                                                                    ...
                                                                    disposition_id="PTP",
                                                                    flag=TIMEOUT_FLAG, ptp_date=given_date,emi_flow="post_emi_7_15",sheet_name=sheet_name)
                                    # send_message(tracker)
                                    return {"availability_status": value, "stop_conversation": "TRUE", "trail_count": None}
                                elif no_of_days.days<2:
                                    given_date = given_date.strftime("%d %B %Y")
                                    dispatcher.utter_template("utter_accepted_ptp_post_due",tracker,ptp_day=given_date)
                                    send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher,
                                                                    user_message=user_message,
                                                                    disposition_id="PTP",
                                                                    flag=TIMEOUT_FLAG, ptp_date=given_date,emi_flow="post_emi_7_15",sheet_name=sheet_name)
                                    ...
                                    send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher,
                                                                    user_message=user_message,
                                                                    disposition_id="PTP",
                                                                    flag=DEFAULT_FLAG, ptp_date=given_date,emi_flow="post_emi_7_15",sheet_name=sheet_name)
                                    # send_message(tracker)
                        ...
                        #     send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,
                        #                                     disposition_id="PTP", flag=TIMEOUT_FLAG,emi_flow="post_emi_7_15",sheet_name=sheet_name)
                        #     return {"availability_status": value, "stop_conversation": "TRUE", "trail_count": None}
            else:
                send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,
                                                disposition_id="PTP", flag=DEFAULT_FLAG,emi_flow="post_emi_7_15",sheet_name=sheet_name)
                return {"availability_status": "confirm", "trail_count": None}
            ...
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,disposition_id="delay reason",
                                               ...
                                               flag=DEFAULT_FLAG,emi_flow="post_emi_7_15",sheet_name=sheet_name)
            return {"availability_status": "confirm"}
    ...
    def validate_payment_confirmation(
        ...
        if value=="FALSE":
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,
                                               disposition_id="rtp", flag=DEFAULT_FLAG,emi_flow="post_emi_7_15",sheet_name=sheet_name)
            return {"payment_confirmation": "decline"}
        elif value=="TRUE":
            entities = tracker.latest_message["entities"]
            now = datetime.datetime.now()
            intent = tracker.latest_message.get("intent").get("name")
            ...
            if entities:
                    ...
                    for entity in entities:
                        if entity.get("entity", None) == "date":
                            ...
                            if given_date:
                                ...
                                if no_of_days<2:
                                    given_date = given_date.strftime("%d %B %Y")
                                    dispatcher.utter_template("utter_accepted_ptp_post_due",tracker)
                                    send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher,
                                                                    user_message=user_message,
                                                                    disposition_id="PTP",
                                                                    flag=TIMEOUT_FLAG, ptp_date=given_date,emi_flow="post_emi_7_15",sheet_name=sheet_name)
                ...
                send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher,
                                                user_message=user_message,
                                                disposition_id="PTP",
                                                flag=TIMEOUT_FLAG, emi_flow="post_emi_7_15",sheet_name=sheet_name)
                # send_message(tracker)
                return {"payment_confirmation": value, "stop_conversation": "TRUE", "trail_count": None}
        elif value == "decline_reason":
            dispatcher.utter_template("utter_not_accepted_reason_post_due8_15", tracker)
            intent = tracker.latest_message.get("intent").get("name")
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,disposition_id="delay reason",
                                               delay_reason=decline_reason_disposition_id.get(intent),
                                               flag=TIMEOUT_FLAG,emi_flow="post_emi_7_15",sheet_name=sheet_name)
    ...
    def validate_ask_delay_reason(
        ...
        if value == "FALSE":
            ...
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,disposition_id="RTP",
                                               flag=TIMEOUT_FLAG, delay_reason=decline_reason_disposition_id.get(intent),emi_flow="post_emi_7_15",sheet_name=sheet_name)
            ...
            send_and_store_disposition_details(tracker=tracker, dispatcher=dispatcher, user_message=user_message,disposition_id="delay reason",
                                               flag=TIMEOUT_FLAG, emi_flow="post_emi_7_15",sheet_name=sheet_name)
            return {"ask_delay_reason": None, "stop_conversation": "TRUE"}
