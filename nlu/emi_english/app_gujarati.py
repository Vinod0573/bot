...
def webhook():
    ...
    if str(msg).lower() == "hello" or str(msg).lower() == "hey":
        ...
        # entity_response = requests.post(' http://52.168.172.23/guj_ner/entity/parse',json={"text": str(msg)}).json()
        try:
            entity_response = requests.post('http://52.147.223.178:80/api/v1/service/production-ner/score',json={"data": str(msg),"lang":"gujrati"}).json()
        except:
            logging.error("Gujarati NER link error")
