...
logging.basicConfig(filename="NLU_tamil.log",filemode="w",format='%(name)s - %(asctime)s - %(message)s')
...
def webhook():
    ...
    if str(msg).lower() == "hello" or str(msg).lower() == "hey":
        ...
        try:
            entity_response = requests.post('http://52.147.223.178:80/api/v1/service/production-ner/score',json={"data": str(msg),"lang":"tamil"}).json()
        except:
            logging.error("Tamil NER link error")
