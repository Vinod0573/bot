...
def webhook():
    ...
    if str(msg).lower() == "hello" or str(msg).lower() == "hey":
        ...
        try:
            entity_response = requests.post('http://52.151.242.140:80/api/v1/service/testing-ner/score',json={"data":str(msg), "lang": "telugu"}).json()
        except:
            logging.error("Telugu NER link error")
