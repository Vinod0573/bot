...
logging.basicConfig(filename="NLU_punjabi.log",filemode="w",format='%(name)s - %(asctime)s - %(message)s')
...
def webhook():
    ...
    if str(msg).lower() == "hello" or str(msg).lower() == "hey":
        ...
        try:
            entity_response = requests.post('http://52.147.223.178:80/api/v1/service/production-ner/score',json={"data": str(msg),"lang":"punjabi"}).json()
        except:
