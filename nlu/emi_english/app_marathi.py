...
logging.basicConfig(filename="NLU_marathi.log",filemode="w",format='%(name)s - %(asctime)s - %(message)s')
...
def get_duckling_entities(message):
    try:
        ...
        logging.error("Duckling link error")
        return []
...
def webhook():
    ...
    if str(msg).lower() == "hello" or str(msg).lower() == "hey":
        ...
        try:
            entity_response = requests.post('http://52.147.223.178:80/api/v1/service/production-ner/score',json={"data": str(msg),"lang":"marathi"}).json()
        except:
            logging.error("Marathi NER link error")
